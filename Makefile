...
logcopter-check:
	GOWORK=off go tool logcopter-gen -area-prefix go-go-golems.researchctl -strip-prefix github.com/go-go-golems/researchctl -check ./pkg/...
...
release:
	...
	GOWORK=off GOPROXY=proxy.golang.org go list -m github.com/go-go-golems/researchctl@$(shell svu current)
...
RESEARCHCTL_BINARY=$(shell which researchctl)
install:
	GOWORK=off go build -o ./dist/researchctl ./cmd/researchctl && \
		cp ./dist/researchctl $(RESEARCHCTL_BINARY)
